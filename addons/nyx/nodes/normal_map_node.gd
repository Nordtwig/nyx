...
func get_uniform_name() -> String:
	return _uniform_name if _uniform_name.strip_edges() != "" else "nrm_" + str(name).to_lower()


func get_uniform_declaration() -> String:
	return "uniform sampler2D %s : hint_normal;" % get_uniform_name()
